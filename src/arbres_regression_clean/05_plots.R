...
# ---------------------------------------------------------------------
plot_learning_curve <- function(lc_summary, subtitle = "") {
  ggplot(lc_summary, aes(x = fraction)) +
...
# ---------------------------------------------------------------------
plot_obs_vs_pred_scatter <- function(obs_pred_all, subtitle = "") {
  ...
  ggplot(obs_pred_all, aes(x = obs, y = pred, color = factor(fold_id))) +
    ...
    annotate("label", x = min(obs_pred_all$obs, na.rm = TRUE), y = max(obs_pred_all$pred, na.rm = TRUE),
...
plot_obs_vs_pred_by_fold <- function(obs_pred_all, subtitle = "") {
  ggplot(obs_pred_all, aes(x = obs, y = pred)) +
...
# ---------------------------------------------------------------------
plot_importance_mean_sd <- function(importance_all, subtitle = "") {
  summary_imp <- importance_all %>%
    group_by(variable) %>%
    summarise(mean_imp = mean(importance, na.rm = TRUE),
              sd_imp   = sd(importance, na.rm = TRUE), .groups = "drop")

  ggplot(summary_imp, aes(x = reorder(variable, mean_imp), y = mean_imp)) +
...
# ---------------------------------------------------------------------
plot_residual_map <- function(obs_pred_all, subtitle = "") {
...
plot_abs_residual_map <- function(obs_pred_all, subtitle = "") {
...
# ---------------------------------------------------------------------
plot_metric_bar <- function(metrics_all, metric, fill = "steelblue", hline0 = FALSE, subtitle = "") {
